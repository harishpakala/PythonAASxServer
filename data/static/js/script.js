function createProductionStepTags(canvasId,productionStepName,stepSequence){
	...
	context.beginPath();
	if (productionStepName === "BoringRequester" || productionStepName === "HoningRequester"){
		context.rect(centerX-radius*2, centerY-radius, canvas.width - radius*5, radius*4);
	}
	else{
	...
	const lineheight = 15;
	const lines = productionStepName.split('\n');

	for (let i = 0; i < lines.length; i++) {
	  context.fillText(lines[i], centerX+2*radius, centerY+radius*0.25 + (i * lineheight));
	}
	...
	if ( typeof connectstatusDict === "object")
		{
			for (const [key, value] of Object.entries(connectstatusDict)) {
				...
				if (value["type"] == "collection")
				{
						...
						objectli.setAttribute("onclick","('"+aasIndex+"','"+submodel+"',"+JSON.stringify(value["qualifierList"]) +","+JSON.stringify(value["semanticIdList"])+",'"+key+"','"+exDomain+"','"+value["idShortPath"]+"','"+value["identificationId"]+"');event.stopPropagation()");
