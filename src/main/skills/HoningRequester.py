...
class WaitforTransportOrderCompletion:
    ...
    def WaitforTransportOrderCompletion_Logic(self):
        ...
        """
        try:
            statusMessage = self.baseClass.WaitforTransportOrderCompletion_In["interactionElements"][0]
            statusResponse = statusMessage["submodelElements"][0]["value"]
            if (statusResponse == "E"):
                self.baseClass.responseMessage["status"] = "E"
                self.baseClass.responseMessage["code"] = statusMessage["submodelElements"][1]["value"]
                self.baseClass.responseMessage["message"] =  statusMessage["submodelElements"][2]["value"]
                self.WaitforInformConfirm_Enabled = False
            else:
                self.sendCompletionResponse_Enabled = False
        except Exception as e:
            self.baseClass.responseMessage["status"] = "E"
            self.baseClass.responseMessage["code"] = "E.014"
            self.baseClass.responseMessage["message"] =  "Error Processing the Order"
            self.WaitforInformConfirm_Enabled = False
...
class sendTransportOrder:
    ...
    def sendTransportOrder_Logic(self):
        ...
        """
        try:
            #acceptproposalMessageList = list(self.baseClass.sendacceptProposal_Queue.queue) # in case for further processing is required
            acceptproposalMessage = self.baseClass.acceptProposal#acceptproposalMessageList[0]
        ...
        j = 0
        k = 0
        self.TransportSubmodel,status,statuscode = self.baseClass.pyAAS.dba.GetSubmodelById("www.company.com/ids/sm/0364_8256_7746_9243")
        ...
        try:
            ...
            self.baseClass.skillLogger.info("Error" + str(e))   
    ...
    def create_Outbound_Message(self) -> list:
        ...
        for oMessage in self.oMessages:
            ...
            # For sending the message to an internal skill
            # The receiver Id should be
            psp = ProductionStepOrder(self.baseClass.pyAAS)
            ...
            I40FrameData =      {
                                    ...
                                    "SenderRolename" : "HoningRequester",
                                    "conversationId" : psp.createTransportStepOrder(self.baseClass.aasIndex,currentConvId),
...
class SendCFP:
    ...
    def create_Outbound_Message(self) -> list:
        ...
        for oMessage in self.oMessages:
            ...
            # the relevant submodel could be retrieved using
            # interactionElements
            submodelIdentifier = message["interactionElements"][0]
            submodel,status,statuscode = self.base_class.pyaas.dba.GetSubmodelById(submodelIdentifier)
            oMessage_Out ={"frame": self.frame,
...
class HoningRequester:
    ...
    def __init__(self,pyaas):
        ...
        self.pyaas = pyaas
        self.skillName = "HoningRequester"
