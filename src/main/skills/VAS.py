...
class MappingVerification:
    ...
    def __init__(self, base_class):
        ...
        self.NotifyFeedBacktoVoR_Enabled = True
        self.AcceptanceAndRequestExecution_Enabled = True
    

    def MappingVerification_Logic(self):
        """
            The actualy logic this state  goes into this method.
            It is upto the developer to add the relevant code.
        """
        self.NotifyFeedBacktoVoR_Enabled = False
    ...
    def run(self) -> None:
        ...
        # StartState
        self.base_class.skillLogger.info("StartState: MappingVerification")
        # InputDocumentType"
        InputDocument = "NA"
        self.base_class.skillLogger.info("InputDocument : " + InputDocument)
        
        self.MappingVerification_Logic()
    ...
    def next(self) -> object:
        ...
        if (self.NotifyFeedBacktoVoR_Enabled):
            self.base_class.skillLogger.info("Condition :" + "true")
            ts = NotifyFeedBacktoVoR(self.base_class)
            self.base_class.skillLogger.info("TargettState: " + ts.__class__.__name__)
            self.base_class.skillLogger.info("############################################################################# \n")
            return ts
        if (self.AcceptanceAndRequestExecution_Enabled):
            self.base_class.skillLogger.info("Condition :" + "true")
            ts = AcceptanceAndRequestExecution(self.base_class)
            self.base_class.skillLogger.info("TargettState: " + ts.__class__.__name__)
            self.base_class.skillLogger.info("############################################################################# \n")
            return ts
...
class WaitForRequests:
    ...
    def retrieve_WaitForRequests_Message(self) -> None:
        """
            ...
            current state.
        """
        self.base_class.WaitForRequests_In = self.base_class.WaitForRequests_Queue.get()
    
    def saveMessage(self) -> None:
        """
            Method to save the message into the database
        """
        inboundQueueList = list(self.base_class.WaitForRequests_Queue.queue) # in case for further processing is required
        # else creation of the new queue is not required.
        for i in range (0, self.base_class.WaitForRequests_Queue.qsize()):
            message = inboundQueueList[i]
            self.instanceId = str(uuid.uuid1())
            edm = ExecuteDBModifier(self.base_class.pyaas)
            data,status,statuscode = edm.execute({"data":{"elemData":message["interactionElements"][0],
                                            "submodelIdentifier" : "https://example.com/ids/sm/5282_8121_2122_2515"
    ...
    def WaitForRequests_Logic(self):
        """
            The actualy logic this state  goes into this method.
            It is upto the developer to add the relevant code.
        """
    ...
    def run(self) -> None:
        ...
        self.base_class.skillLogger.info("\n #############################################################################")
        # StartState
        self.base_class.skillLogger.info("StartState: WaitForRequests")
        # InputDocumentType"
        InputDocument = "vorrequest"
        self.base_class.skillLogger.info("InputDocument : " + InputDocument)
        ...
        if (InputDocument != "NA"):
            ...
            sys.stdout.flush()
            while (((self.base_class.WaitForRequests_Queue).qsize()) == 0):
                time.sleep(1)
            if (self.messageExist):
                self.saveMessage() # in case we need to store the incoming message
                self.retrieve_WaitForRequests_Message() # in case of multiple inbound messages this function should 
                                                      # not be invoked. 
        self.WaitForRequests_Logic()
    ...
    def next(self) -> object:
        ...
        if (self.Mapping_Enabled):
            self.base_class.skillLogger.info("Condition :" + "true")
            ts = Mapping(self.base_class)
            self.base_class.skillLogger.info("TargettState: " + ts.__class__.__name__)
            self.base_class.skillLogger.info("############################################################################# \n")
            return ts
        
class NotifyFeedBacktoVoR:
    ...
    def __init__(self, base_class):
        ...
        self.WaitForRequests_Enabled = True
        self.WaitForRequests_Enabled = True
    

    def NotifyFeedBacktoVoR_Logic(self):
        """
            The actualy logic this state  goes into this method.
            It is upto the developer to add the relevant code.
        """
    ...
    def create_Outbound_Message(self) -> list:
        ...
        """
        self.oMessages = "vasfeedback".split("/")
        outboundMessages = []
        for oMessage in self.oMessages:
            message = self.base_class.WaitForRequests_In
            self.gen = Generic()
    ...
    def run(self) -> None:
        ...
        self.base_class.skillLogger.info("\n #############################################################################")
        # StartState
        self.base_class.skillLogger.info("StartState: NotifyFeedBacktoVoR")
        # InputDocumentType"
        InputDocument = "NA"
        self.base_class.skillLogger.info("InputDocument : " + InputDocument)
        
        self.NotifyFeedBacktoVoR_Logic()
    ...
    def next(self) -> object:
        ...
        if (self.WaitForRequests_Enabled):
            self.base_class.skillLogger.info("Condition :" + "NA")
            ts = WaitForRequests(self.base_class)
            self.base_class.skillLogger.info("TargettState: " + ts.__class__.__name__)
            self.base_class.skillLogger.info("############################################################################# \n")
            return ts
        if (self.WaitForRequests_Enabled):
...
class Mapping:
    ...
    def __init__(self, base_class):
        ...
        self.NotifyFeedBacktoVoR_Enabled = True
        self.MappingVerification_Enabled = True
    ...
    def run(self) -> None:
        ...
        self.base_class.skillLogger.info("\n #############################################################################")
        # StartState
        self.base_class.skillLogger.info("StartState: Mapping")
        # InputDocumentType"
        InputDocument = "NA"
        self.base_class.skillLogger.info("InputDocument : " + InputDocument)
    ...
    def next(self) -> object:
        ...
        if (self.NotifyFeedBacktoVoR_Enabled):
            self.base_class.skillLogger.info("Condition :" + "Mapping")
            ts = NotifyFeedBacktoVoR(self.base_class)
            self.base_class.skillLogger.info("TargettState: " + ts.__class__.__name__)
            self.base_class.skillLogger.info("############################################################################# \n")
            return ts
        if (self.MappingVerification_Enabled):
            self.base_class.skillLogger.info("Condition :" + "true")
            ts = MappingVerification(self.base_class)
            self.base_class.skillLogger.info("TargettState: " + ts.__class__.__name__)
            self.base_class.skillLogger.info("############################################################################# \n")
            return ts
        
class AcceptanceAndRequestExecution:
    ...
    def AcceptanceAndRequestExecution_Logic(self):
        """
            The actualy logic this state  goes into this method.
            It is upto the developer to add the relevant code.
        """
    ...
    def run(self) -> None:
        ...
        self.base_class.skillLogger.info("\n #############################################################################")
        # StartState
        self.base_class.skillLogger.info("StartState: AcceptanceAndRequestExecution")
        # InputDocumentType"
        InputDocument = "NA"
        self.base_class.skillLogger.info("InputDocument : " + InputDocument)
        
        self.AcceptanceAndRequestExecution_Logic()
...
class VAS:
    ...
    def initstate_specific_queue_internal(self) -> None:
        ...
        self.MappingVerification_Queue = Queue.Queue()
        self.WaitForRequests_Queue = Queue.Queue()
        self.NotifyFeedBacktoVoR_Queue = Queue.Queue()
        self.Mapping_Queue = Queue.Queue()
        self.AcceptanceAndRequestExecution_Queue = Queue.Queue()
        self.Start_Queue = Queue.Queue()
        
                
        self.QueueDict = {
              "vorrequest": self.WaitForRequests_Queue,
            }
    
    def init_inbound_messages(self) -> None:
        self.WaitForRequests_In = {}
        pass
    ...
    def __init__(self,pyaas):
        ...
        self.SKILL_STATES = {
                          "MappingVerification": "MappingVerification",  "WaitForRequests": "WaitForRequests",  "NotifyFeedBacktoVoR": "NotifyFeedBacktoVoR",  "Mapping": "Mapping",  "AcceptanceAndRequestExecution": "AcceptanceAndRequestExecution",  "Start": "Start",
                       }
        ...
        self.skill_service = "Request Automation"
        self.gen = Generic()
