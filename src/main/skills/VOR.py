...
class Start:
    ...
    def __init__(self, base_class):
        ...
        self.WaitForRequest_Enabled = True
...
class WaitforVASFeedBack:
    ...
    def retrieve_WaitforVASFeedBack_Message(self) -> None:
        """
            ...
            current state.
        """
        self.base_class.WaitforVASFeedBack_In = self.base_class.WaitforVASFeedBack_Queue.get()
    
    def saveMessage(self) -> None:
        """
            Method to save the message into the database
        """
        inboundQueueList = list(self.base_class.WaitforVASFeedBack_Queue.queue) # in case for further processing is required
        # else creation of the new queue is not required.
        for i in range (0, self.base_class.WaitforVASFeedBack_Queue.qsize()):
            message = inboundQueueList[i]
            self.instanceId = str(uuid.uuid1())
            self.base_class.pyaas.dataManager.pushInboundMessage({"functionType":3,"instanceid":self.instanceId,
                                                            "conversationId":message["frame"]["conversationId"],
    ...
    def WaitforVASFeedBack_Logic(self):
        """
            The actualy logic this state  goes into this method.
            It is upto the developer to add the relevant code.
        """
    ...
    def run(self) -> None:
        ...
        self.base_class.skillLogger.info("\n #############################################################################")
        # StartState
        self.base_class.skillLogger.info("StartState: WaitforVASFeedBack")
        # InputDocumentType"
        InputDocument = "vasfeedback"
        self.base_class.skillLogger.info("InputDocument : " + InputDocument)
        ...
        if (InputDocument != "NA"):
            ...
            sys.stdout.flush()
            while (((self.base_class.WaitforVASFeedBack_Queue).qsize()) == 0):
                time.sleep(1)
                i = i + 1 
                if i > 25: # Time to wait the next incoming message
            ...
            if (self.messageExist):
                ...
                self.retrieve_WaitforVASFeedBack_Message() # in case of multiple inbound messages this function should 
                                                      # not be invoked. 
        self.WaitforVASFeedBack_Logic()
...
class NotifyFeedBacktoMnO:
    ...
    def create_Outbound_Message(self) -> list:
        ...
        """
        self.oMessages = "feedback".split("/")
        outboundMessages = []
        for oMessage in self.oMessages:
            message = self.base_class.WaitForRequest_In
            self.gen = Generic()
...
class WaitForRequest:
    ...
    def __init__(self, base_class):
        ...
        self.VerifyRequest_Enabled = True
    
    def retrieve_WaitForRequest_Message(self) -> None:
        """
            Method to retrieve the inbound i4.0 message from the relevant queue.
            The retrieved message is assigned to the dictionary variabel designed
            in the base clase. The Variable and the queue name are based on the 
            current state.
        """
        self.base_class.WaitForRequest_In = self.base_class.WaitForRequest_Queue.get()
    
    def saveMessage(self) -> None:
        """
            Method to save the message into the database
        """
        inboundQueueList = list(self.base_class.WaitForRequest_Queue.queue) # in case for further processing is required
        # else creation of the new queue is not required.
        for i in range (0, self.base_class.WaitForRequest_Queue.qsize()):
            message = inboundQueueList[i]
            self.instanceId = str(uuid.uuid1())
            self.base_class.pyaas.dataManager.pushInboundMessage({"functionType":3,"instanceid":self.instanceId,
                                                            "conversationId":message["frame"]["conversationId"],
    ...
    def run(self) -> None:
        ...
        self.base_class.skillLogger.info("\n #############################################################################")
        # StartState
        self.base_class.skillLogger.info("StartState: WaitForRequest")
        # InputDocumentType"
        ...
        if (InputDocument != "NA"):
            ...
            sys.stdout.write(" Waiting for response")
            sys.stdout.flush()
            while (((self.base_class.WaitForRequest_Queue).qsize()) == 0):
                time.sleep(1)
            if (self.messageExist):
                self.saveMessage() # in case we need to store the incoming message
                self.retrieve_WaitForRequest_Message() # in case of multiple inbound messages this function should 
                                                    # not be invoked. 
        self.WaitForRequest_Logic()
    ...
    def next(self) -> object:
        ...
        if (self.VerifyRequest_Enabled):
            self.base_class.skillLogger.info("Condition :" + "true")
            ts = VerifyRequest(self.base_class)
            self.base_class.skillLogger.info("TargettState: " + ts.__class__.__name__)
            self.base_class.skillLogger.info("############################################################################# \n")
            return ts
...
class VerifyRequest:
    ...
    def VerifyRequest_Logic(self):
        """
            The actualy logic this state  goes into this method.
            It is upto the developer to add the relevant code.
        """
        self.NotifyFeedBacktoMnO_Enabled = False
    ...
    def run(self) -> None:
        ...
        self.base_class.skillLogger.info("\n #############################################################################")
        # StartState
        self.base_class.skillLogger.info("StartState: VerifyRequest")
        # InputDocumentType"
        InputDocument = "NA"
        self.base_class.skillLogger.info("InputDocument : " + InputDocument)
        
        self.VerifyRequest_Logic()
    ...
    def next(self) -> object:
        ...
        if (self.Propagation_Enabled):
            self.base_class.skillLogger.info("Condition :" + "true")
            ts = Propagation(self.base_class)
            self.base_class.skillLogger.info("TargettState: " + ts.__class__.__name__)
            self.base_class.skillLogger.info("############################################################################# \n")
            return ts
        if (self.NotifyFeedBacktoMnO_Enabled):
...
class Propagation:
    ...
    def __init__(self, base_class):
        ...
        self.WaitforVASFeedBack_Enabled = True
    

    def Propagation_Logic(self):
        """
            The actualy logic this state  goes into this method.
            It is upto the developer to add the relevant code.
        """
    ...
    def create_Outbound_Message(self) -> list:
        """
            ...
            The message type is carried from the json file.
        """
        self.oMessages = "vorrequest".split("/")
        outboundMessages = []
        for oMessage in self.oMessages:
            message = self.base_class.WaitForRequest_In
            self.gen = Generic()
            ...
            # For the return reply these details could be obtained from the inbound Message
            receiverId = "https://example.com/ids/aas/5444_1121_2122_1612"#message["frame"]["sender"]["id"]
            receiverRole = "VAS"#message["frame"]["sender"]["role"]["name"]
            ...
            #submodel,status,statuscode = self.base_class.pyaas.dba.GetSubmodelById("submodelIdentifier")
            requestCollection = self.base_class.WaitForRequest_In["interactionElements"][0]
            
            requestCollection1 =None
            edm = ExecuteDBModifier(self.base_class.pyaas)
            data,status,statuscode = edm.execute({"data":{"elemData":requestCollection,
                                            "submodelIdentifier" : "https://example.com/ids/sm/5324_0121_2122_4530"
                                                                  },"method":"PostSubmodelElement_SRI","instanceId" : str(uuid.uuid1())})
            
            oMessage_Out ={"frame": self.frame,
                                    "interactionElements":[requestCollection]}
            self.instanceId = str(uuid.uuid1())
            self.base_class.pyaas.dataManager.pushInboundMessage({"functionType":3,"instanceid":self.instanceId,
                                                            "conversationId":oMessage_Out["frame"]["conversationId"],
                                                            "messageType":oMessage_Out["frame"]["type"],
    ...
    def run(self) -> None:
        ...
        self.base_class.skillLogger.info("\n #############################################################################")
        # StartState
        self.base_class.skillLogger.info("StartState: Propagation")
        # InputDocumentType"
        InputDocument = "NA"
        self.base_class.skillLogger.info("InputDocument : " + InputDocument)
        
        self.Propagation_Logic()
    ...
    def next(self) -> object:
        ...
        if (self.WaitforVASFeedBack_Enabled):
            self.base_class.skillLogger.info("Condition :" + "true")
            ts = WaitforVASFeedBack(self.base_class)
            self.base_class.skillLogger.info("TargettState: " + ts.__class__.__name__)
            self.base_class.skillLogger.info("############################################################################# \n")
            return ts
...
class VOR:
    ...
    def initstate_specific_queue_internal(self) -> None:
        ...
        self.WaitforVASFeedBack_Queue = Queue.Queue()
        self.NotifyFeedBacktoMnO_Queue = Queue.Queue()
        self.WaitForRequest_Queue = Queue.Queue()
        self.VerifyRequest_Queue = Queue.Queue()
        self.Propagation_Queue = Queue.Queue()
        
                
        self.QueueDict = {
              "vasfeedback": self.WaitforVASFeedBack_Queue,
              "request": self.WaitForRequest_Queue,
            }
    
    def init_inbound_messages(self) -> None:
        self.WaitforVASFeedBack_In = {}
        self.WaitForRequest_In = {}
        pass
    ...
    def __init__(self,pyaas):
        ...
        self.SKILL_STATES = {
                          "Start": "Start",  "WaitforVASFeedBack": "WaitforVASFeedBack",  "NotifyFeedBacktoMnO": "NotifyFeedBacktoMnO",  "WaitForRequest": "WaitForRequest",  "VerifyRequest": "VerifyRequest",  "Propagation": "Propagation",
                       }
        ...
        self.skill_service = "Verification of Request"
        self.gen = Generic()
