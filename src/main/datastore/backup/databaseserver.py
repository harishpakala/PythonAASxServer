...
try:
    from config.aasxconfig import StandardSubmodelExtractor
except ImportError:
    from config.aasxconfig import StandardSubmodelExtractor
...
class AASShellParser(object):
    def __init__(self,aasHashDict,submodelHashDict,aasShellHashDict,aasconfig,dba):
        self.aasHashDict = aasHashDict
        self.submodelHashDict = submodelHashDict
        self.aasShellHashDict = aasShellHashDict
        self.dba = dba
        self.aasconfig = aasconfig
        self.uuidG = UUIDGenerator()    
        self.sse  = StandardSubmodelExtractor(aasHashDict,submodelHashDict,aasShellHashDict,self.aasconfig)
        
    def attach_standard_submodels(self,_element,_aasElementObject,shellIndex):
        for submodel in _element["submodels"]:
            _submodelid  = submodel["keys"][0]["value"]
            if self.aasHashDict.__isKeyPresent__(_submodelid):
                _id = self.aasHashDict.__getHashEntry__(_submodelid)._id
                data,status, code = self.dba.GetSubmodelById(_submodelid)
                try:
                    semanticId = data["semanticId"]["keys"][0]["value"]
                    if status :
                        if semanticId == "https://admin-shell.io/zvei/nameplate/1/0/Nameplate":
                            _aasElementObject.add_standard_submodel("https://admin-shell.io/zvei/nameplate/1/0/Nameplate",self.sse.get_nameplate(data,_submodelid),"NAMEPLATE")
                        elif semanticId == "https://www.hsu-hh.de/aut/aas/identification":
                            _aasElementObject.add_standard_submodel("https://www.hsu-hh.de/aut/aas/identification",self.sse.get_identification(data,_submodelid),"IDENTIFICATION")
                        elif semanticId == "http://admin-shell.io/vdi/2770/1/0/Documentation":
                            _aasElementObject.add_standard_submodel("http://admin-shell.io/vdi/2770/1/0/Documentation",self.sse.get_technicaldata(data, _submodelid),"DOCUMENTATION")
                        elif semanticId == "http://admin-shell.io/sandbox/SG2/TechnicalData/Submodel/1/1":
                            _aasElementObject.add_standard_submodel("http://admin-shell.io/sandbox/SG2/TechnicalData/Submodel/1/1",self.sse.get_documentation(data,_submodelid),"TECHNICAL_DATA")
                        elif semanticId == "https://www.w3.org/2019/wot/td#Thing":
                            _aasElementObject.add_standard_submodel("https://www.w3.org/2019/wot/td#Thing",self.sse.get_asset_interface_description(data,_element["id"],shellIndex),"ThingDescription")
                        elif data["idShort"] == "OperationData":
                            _aasElementObject.add_standard_submodel("OperationData",self.sse.get_skills(data,_submodelid,_element["id"],_element["idShort"],shellIndex),"OperationData")
                except Exception as e:
                    pass
        return _aasElementObject
    ...
    def register(self,_newId,_element):
        ...
        _aasElementObject = self.attach_standard_submodels(_element,_aasElementObject,shellIndex)
...
class AAS_Database_Server(object):
    ...
    def parseAssetAdministrationShells(self,assetAdministrationShells):
        for _aasShell in assetAdministrationShells:
            aasShellObject = AASShellParser(self.aasHashDict,self.submodelHashDict,self.aasShellHashDict,self.pyAAS.aasConfigurer,self)
    ...
    def GetAllSubmodelElements(self,data):
        try:
            _submodel,status,statuscode = self.GetSubmodel(data)
            if (status):
    ...
    def GetSubmodelElementByPath(self,data):
        try:
            _shellId = data ["_shellId"]
            _submodelIdentifier = data ["submodelIdentifier"]
            referencePresent = False
            if self.aasHashDict.__isKeyPresent__(_shellId):
                ...
                if (referencePresent):
                    if self.aasHashDict.__isKeyPresent__(_submodelIdentifier):
                        _idSHortPath = _submodelIdentifier +"." +data["idShortPath"]
                        if self.aasHashDict.__isKeyPresent__(_idSHortPath):
    ...
    def GetSubmodelElementByPath_History(self,data):
        try:
            _shellId = data ["_shellId"]
            _submodelIdentifier = data ["submodelIdentifier"]
            referencePresent = False
    ...
    def PostAssetAdministrationShell(self,_aasShell):
        try:
            ...
            self.pyAAS.serviceLogger.info("Error at postAASShell DB" + str(E))
            return  "Internal Server Error", False,500    
   ...
   Submodel Repository Interface Start
    ...
    def GetSubmodelElementByPath_SRI(self,data):
        try:
            _submodelIdentifier = data ["submodelIdentifier"]
            if self.aasHashDict.__isKeyPresent__(_submodelIdentifier):
                _idSHortPath = _submodelIdentifier +"." +data["idShortPath"]
    ...
    def GetFileByPath_SRI(self,data):
        try:
            _submodelIdentifier = data ["submodelIdentifier"]
            if self.aasHashDict.__isKeyPresent__(_submodelIdentifier):
                _idSHortPath = _submodelIdentifier +"." +data["idShortPath"]
    ...
    def get_aas_information(self,uuid) -> dict():
        returnDict = dict()
        _aasShell = self.aasShellHashDict.__getHashEntry__(uuid)
        stdSubmodelList = []
        for key in _aasShell.standardSubmodels.keys():
            stdSubmodelList.append(_aasShell.standardSubmodels[key][1])
        returnDict["stdSubmodelList"] = stdSubmodelList
        returnDict["skillList"] = list(_aasShell.skills_List)
